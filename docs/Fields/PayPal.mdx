...
## Create PayPal Config 
 ...
 <Image src="PayPal/1.App-Settings-to-Payment-Bit-Form" alt="paypal" w="1020" h="300" /> 
 ...
 4. Admin have to create an application from the `PayPal Developer` site. For that log-in to your `PayPal Developer` account & select `My apps & credentials`.
 ...
 5. There are two types of app - `Sandbox Mode` & `Live mode`. For the demonstration purpose we have selected `Sandbox` as an app. You have to select `Live` for the real-time transaction.

 <Image src="PayPal/6.App-Mode-Bit-Form" alt="paypal" w="1020" h="220" /> 
  ...
  ***Set `App Name` => Select `App Type` => Click on `Create App` => Get `Client Id`.***
  ...
  <Image src="PayPal/7.Create-App-Bit-Form" alt="paypal" w="1020" h="300" /> 

  <Image src="PayPal/8.Create-App-2-Bit-Form" alt="paypal" w="1020" h="300" /> 

 8. Now, copy the client id & go back to Bit Form PayPal settings page. Then paste the it on the `Client  Id` field. 
 
 <Image src="PayPal/9.Copy-Client-Id-Bit-Form" alt="paypal" w="1020" h="300" /> 

 <Image src="PayPal/10.Paste-Client-Id-Bit-Form" alt="paypal" w="1020" h="300" /> 

 9. Complete the PayPal config settings by clicking on `Save` button.

 <Image src="PayPal/11.Save-Bit-Form" alt="paypal" w="1020" h="300" /> 
 
 10. Go back to your form and select the created PayPal config.
